...
int main(int argc, char **argv)
{
    ...
    int optRCVLOWAT = readbyte;
    setsockopt(Socket_Server.client_socket, SOL_SOCKET, SO_RCVLOWAT, &optRCVLOWAT, sizeof(optRCVLOWAT));
    ...
    {
        // testset = readset;
        FD_SET(Socket_Server.client_socket, &readset);
        ...
        if (rets < 0) //处理定时器信号
        {
            ...
            if (errno == EINTR)
            {
                alarm(wdelay);
                int n;
                n = write(Socket_Server.client_socket, Socket_Server.buff, writebyte);
                if (debug == 1)
        ...
        {
            if (FD_ISSET(Socket_Server.client_socket, &readset))
            {
                /*进入read状态*/
                int n;
