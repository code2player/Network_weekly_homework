...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        FD_SET(Socket_Client.client_socket, &writeset); //向set中添加server端socket
        /*select函数的运用*/
        int ret1;
        ret1 = select(FD_SETSIZE, 0, &writeset, 0, NULL);
        // ret2 = select(FD_SETSIZE, 0, 0, 0, NULL);
        cout << "select return: " << ret1 << endl;
        printf("select error: %s(errno:%d)\n", strerror(errno), errno);
        if (FD_ISSET(Socket_Client.client_socket, &writeset))
        {
            int n;
            n = write(Socket_Client.client_socket, Socket_Client.buff, 100);
            printf("write error: %s(errno:%d)\n", strerror(errno), errno);
            cout << "write" << n << "个字节" << endl;
            if (n < 0)
            {
                cout << "write失败, getchar暂停, 按回车键继续" << endl;
                getchar();
            }
        }
    }
