...
/*--------------------------------------------------------------*/
    ...
    /*开始进行tcp阻塞方式client端的编写*/
    Socket_Connect Socket_Client1;
    /*client端初始化*/
    Socket_Client1.fill_ip_port(pn.para[0].para_value*2,pn.para[1].para_string,pn.para[2].para_value);
    Socket_Client1.create_client_socket();
    Socket_Client1.client_socket_NONBLOCK(1);
    Socket_Client1.bind_client_socket();
    //Socket_Client.connect_client_socket();
    ...
    while (1)
    {
        int ret2=connect(Socket_Client1.client_socket, (struct sockaddr *)&(Socket_Client1.remote_addr), sizeof(Socket_Client1.remote_addr));
        if (ret2 == 0)
        {
            printf("已连接到server端\n");
            break;
        ...
        else if (errno == EINPROGRESS)
        {
            FD_ZERO(&readset);
            FD_ZERO(&writeset);
            FD_SET(Socket_Client1.client_socket, &readset);
            FD_SET(Socket_Client1.client_socket, &writeset);
            int select_ret = select(FD_SETSIZE, &readset, &writeset, 0, NULL);
            if(select_ret<0)
            {
                printf("select error: %s(errno:%d)\n", strerror(errno), errno);
            ...
            if (FD_ISSET(Socket_Client1.client_socket, &readset) || FD_ISSET(Socket_Client1.client_socket, &writeset))
            {
                connect(Socket_Client1.client_socket, (struct sockaddr *)&(Socket_Client1.remote_addr), sizeof(Socket_Client1.remote_addr));
                int err = errno;
                if (err == EISCONN)
                {
                    printf("已连接到server端\n");
...
/*--------------------------------------------------------------*/
    ...
    while (1)
    {
        FD_SET(Socket_Client.client_socket, &readset); //向set中添加socket0
        FD_SET(Socket_Client1.client_socket, &readset); //向set中添加socket1
        /*select函数的运用*/
        int ret1;
        ...
        if (ret1 < 0)
        {
            ...
            if (debug == 1)
                cout << "socket0 write" << n << "个字节" << endl;
            if (n <= 0)
            {
                printf("write error: %s(errno:%d)\n", strerror(errno), errno);
            }

            n = write(Socket_Client1.client_socket, Socket_Client1.buff, writebyte);
            if (debug == 1)
                cout << "socket1 write" << n << "个字节" << endl;
            if (n <= 0)
            {
                printf("write error: %s(errno:%d)\n", strerror(errno), errno);
            }
        ...
        else
        {
            if (FD_ISSET(Socket_Client.client_socket, &readset))
            {
                ...
                if (debug == 1)
                    cout << "socket0 read" << n << "个字节" << endl;
                if (n <= 0)
                {
                    printf("read error: %s(errno:%d)\n", strerror(errno), errno);
                }
            }
            if (FD_ISSET(Socket_Client1.client_socket, &readset))
            {
                /*进入read状态*/
                int n;
                n = read(Socket_Client1.client_socket, Socket_Client1.buff, readbyte);
                if (debug == 1)
                    cout << "socket1 read" << n << "个字节" << endl;
                if (n <= 0)
                {
                    printf("read error: %s(errno:%d)\n", strerror(errno), errno);
                }
